...
%PYWIN_HOME%\lib\wget --no-check-certificate -N --output-document="%PYWIN_HOME%\lib\downloads\get_ez_setup.py" https://bootstrap.pypa.io/ez_setup.py
dir
"%PYWIN_HOME%\versions\%$CURRENT_PY%\python.exe" "%PYWIN_HOME%\lib\downloads\get_ez_setup.py" --insecure
cd %$callingDirectory%
IF NOT EXIST "%PYWIN_HOME%\versions\%$CURRENT_PY%\Scripts\easy_install.exe" (
echo easy install install failed
endlocal & exit /b 1
...
echo "%PYWIN_HOME%\versions\%$CURRENT_PY%\python.exe" "%PYWIN_HOME%\lib\downloads\get-pip.py" --cert %$certDir%
"%PYWIN_HOME%\versions\%$CURRENT_PY%\python.exe" "%PYWIN_HOME%\lib\downloads\get-pip.py" --cert %$certDir%
IF NOT EXIST "%PYWIN_HOME%\versions\%$CURRENT_PY%\Scripts\pip.exe" (
...
"%PYWIN_HOME%\versions\%$CURRENT_PY%\python.exe" "%PYWIN_HOME%\lib\downloads\get-pip.py" --proxy %$pipProxy%
IF NOT EXIST "%PYWIN_HOME%\versions\%$CURRENT_PY%\Scripts\pip.exe" (
echo pip install failed
endlocal & exit /b 1
) ELSE (
...
"%PYWIN_HOME%\versions\%$CURRENT_PY%\python.exe" "%PYWIN_HOME%\lib\downloads\get-pip.py" --cert %$certDir% --proxy %$pipProxy%
IF NOT EXIST "%PYWIN_HOME%\versions\%$CURRENT_PY%\Scripts\pip.exe" (
echo pip install failed
endlocal & exit /b 1
) ELSE (
