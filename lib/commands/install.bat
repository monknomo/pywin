...
IF "%PYINSTSTYLE%"=="msi1" (
    IF NOT EXIST "%PYWIN_HOME%\lib\downloads\python-%PYTHONDLVERSION%.msi" (
        IF "%$UNSAFE%"=="1" (
            IF "%$caDirectory"=="" (
                ...
                if errorlevel 1 (
                    ...
                    echo "%PYTHONDLURL%" 
                    del "%PYWIN_HOME%\lib\downloads\python-%PYTHONDLVERSION%.msi"
                    GOTO End
                )        
            ) ELSE (
                "%PYWIN_HOME%\lib\wget.exe" --no-check-certificate --ca-directory=%$caDirectory% --continue --tries=5 --output-document="%PYWIN_HOME%\lib\downloads\python-%PYTHONDLVERSION%.msi" "%PYTHONDLURL%"  
                if errorlevel 1 (
                    echo An error was encountered trying to download Python from
                    echo "%PYTHONDLURL%" 
                    del "%PYWIN_HOME%\lib\downloads\python-%PYTHONDLVERSION%.msi"
    ...
    msiexec /i "%PYWIN_HOME%\lib\downloads\python-%PYTHONDLVERSION%.msi" /qn TARGETDIR="%PYWIN_HOME%\versions\%PYTHONDLVERSION%"
) ELSE (
    IF "%PYINSTSTYLE%"=="exe1" (
        IF NOT EXIST "%PYWIN_HOME%\lib\downloads\python-%PYTHONDLVERSION%.exe" (
            IF "%$UNSAFE%"=="1" (
                IF "%$caDirectory%"=="" (
                    ...
                    if errorlevel 1 (
                        echo An error was encountered trying to download Python from
                        echo "%PYTHONDLURL%" 
                        del "%PYWIN_HOME%\lib\downloads\python-%PYTHONDLVERSION%.exe"
                        GOTO End
                    )
                ) ELSE (
                    "%PYWIN_HOME%\lib\wget.exe" --no-check-certificate --ca-directory=%$caDirectory% --continue --tries=5 --output-document="%PYWIN_HOME%\lib\downloads\python-%PYTHONDLVERSION%.exe" "%PYTHONDLURL%"   
                    if errorlevel 1 (
                        echo An error was encountered trying to download Python from
                        echo "%PYTHONDLURL%" 
                        del "%PYWIN_HOME%\lib\downloads\python-%PYTHONDLVERSION%.exe"
        ...
        "%PYWIN_HOME%\lib\downloads\python-%PYTHONDLVERSION%.exe" /passive TargetDir="%PYWIN_HOME%\versions\%PYTHONDLVERSION%" Include_launcher=0 InstallLauncherAllUsers=0 SimpleInstall=1 Shortcuts=0 AssociateFiles=0
...
IF NOT EXIST "%PYWIN_HOME%\lib\currentVersion.txt" (
