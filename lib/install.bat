...
IF "%PYINSTSTYLE%"=="msi1" (
    ...
    IF "%PYINSTSTYLE%"=="exe1" (
        ...
        rem this might be troublesome, in that python requires some dlls...
        echo Download complete, installing python %PYTHONDLVERSION%
        "%PYWIN_HOME%\lib\downloads\python-%PYTHONDLVERSION%.exe" /quiet TargetDir="%PYWIN_HOME%\versions\%PYTHONDLVERSION%" Include_launcher=0 InstallLauncherAllUsers=0 SimpleInstall=1 Shortcuts=0 AssociateFiles=0
    ) ELSE (
